...
function populateUserInfo(userInfo, loginMatch, isFollowing, options = {}) {
    ...
    if (userInterestDisplay) userInterestDisplay.textContent = `관심사: ${userInfo.interest || '없음'}`;

    // 프로필 이미지 설정
    const profileImageDisplay = document.getElementById('user-image-display');
    if (profileImageDisplay) {
        profileImageDisplay.src = userInfo.profileImage ? `/uploads/profileImages/${userInfo.profileImage}` : '/path/to/default/image.jpg';
