...
router.delete('/unfollow/:followingId', authMiddleWare, async (req, res) => {
    ...
    try {
        ...
        if(!existingFollow){
            return res.status(404).json({ message: '해당 유저를 팔로우하고 있지 않습니다.' });
        }
...
router.get('/followers/:userId', authMiddleWare, async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: '팔로워 목록 조회 중 오류가 발생했습니다.', error: error.message });
    }
});
...
router.get('/following/:userId', authMiddleWare, async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: '팔로잉 목록 조회 중 오류가 발생했습니다.', error: error.message });
    }
});
